...
function getTrackingProviders() {
    const defaultProviders = [{ id: "mapotic", url: POIS_URL }];
...
async function refreshSharkPositions() {
    ...
    for (const provider of providers) {
        ...
        if (!res.ok) {
            throw new Error(`Failed to fetch POIs for provider ${provider.id}: ${res.status} ${res.statusText}`);
        }
        const geojson = await res.json();
        if (!geojson.features) {
            throw new Error(`Unexpected GeoJSON format for provider ${provider.id}: no features array`);
        }
        for (const feature of geojson.features) {
            ...
            const sourceId = String(props.id ?? props.slug ?? props.name ?? "");
            if (!sourceId)
                continue;
    ...
    console.log(`refreshSharkPositions: processed ${processedFeatures} features from ${providers.length} provider(s), inserted ${insertedPoints} new points`);
}
